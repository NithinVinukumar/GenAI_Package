...
class VGG16FeatureExtractor(torch.nn.Module):
    def __init__(self):
        ...
        self.enc_1 = torch.nn.Sequential(*vgg16.features[:4])
        self.enc_2 = torch.nn.Sequential(*vgg16.features[4:9])
        self.enc_3 = torch.nn.Sequential(*vgg16.features[9:16])
...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument("--lambda_vgg", type=float, default=5.0, help="weight for VGG perceptual loss")
    ...
    def backward_G(self):
        ...
        # Perceptual VGG Loss (Structural preservation between input and output)
        self.loss_vgg_A = self.criterionVGG(self.fake_B, self.real_A) * self.opt.lambda_vgg
        self.loss_vgg_B = self.criterionVGG(self.fake_A, self.real_B) * self.opt.lambda_vgg
        # combined loss and calculate gradients
