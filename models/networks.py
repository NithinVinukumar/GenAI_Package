...
class SpatialAttention(nn.Module):
    """Spatial Attention Module to focus on specific regions (like fog)"""
    ...
    def forward(self, x):
        ...
        x_at = torch.cat([avg_out, max_out], dim=1)
        x_at = self.conv1(x_at)
        return x * self.sigmoid(x_at)
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x):
        """Forward function (with skip connections and Spatial Attention)"""
        features = self.conv_block(x)
        attended_features = self.spatial_attention(features)
        out = x + attended_features  # add skip connections
        return out
